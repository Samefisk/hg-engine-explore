...
The MVP shiny reservation system stores up to 10 despawned shiny overworld encounters in the current spawner state. When a shiny slot is cleared by map/object lifecycle cleanup, the reservation keeps its map, terrain pool, species, form, level, and gender. The next successful spawn attempt for the same map and pool consumes the oldest matching reservation before rolling a random encounter, so the same shiny can return without adding save-data scope yet. Gender is applied to the generated battle Pokemon, but the overworld sprite still ignores gender-specific visual variants for now. Gender is rolled by species with the vanilla `PokeSexGetMonsNo` helper rather than the form-aware injected `GrabSexFromSpeciesAndForm` helper, because the form-aware helper was unsafe in the overlay spawn path that runs while walking. The battle-time `MON_DATA_GENDER` write uses a 32-bit local buffer, matching the vanilla Pokemon creation path and avoiding an unaligned stack pointer to `SetMonData`. The current pool set is land, surf, headbutt, and fishing; cave/walking encounters share the land pool in this spawner.
