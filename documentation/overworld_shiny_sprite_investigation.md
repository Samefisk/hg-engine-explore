...
The MVP shiny reservation system stores up to 10 despawned shiny overworld encounters in the current spawner state. When a shiny slot is cleared by map/object lifecycle cleanup, the reservation keeps its map, terrain pool, species, form, and level. The next successful spawn attempt for the same map and pool consumes the oldest matching reservation before rolling a random encounter, so the same shiny can return without adding save-data scope yet. Gender is intentionally out of scope for this MVP because overworld spawn objects do not currently carry gender. The current pool set is land, surf, headbutt, and fishing; cave/walking encounters share the land pool in this spawner.
