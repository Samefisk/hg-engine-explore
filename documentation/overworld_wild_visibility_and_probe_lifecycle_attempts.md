# Visibility, BIT_VANISH, And Probe Lifecycle
...
- Tracks invisibility, BIT_VANISH, helper/probe lifecycles, and the rule that normal Pokemon should be visible from birth.
- BIT_VANISH should be opt-in for phantom stalk only; normal probes and Mankey should not inherit hidden state.
- Separate probe IDs/lifecycles made Charmander diagnostics ambiguous and were replaced with normal spawn-slot handling, then visible land fallback for the immediate test.
