...
def verify_packaged_cache_narcs(repo: Path) -> None:
    ...
    for fragment in (
        ...
        "OVERWORLD_WILD_BEHAVIOR_PROJECTION_TARGET := $(BUILD)/a028/9_21",
        "NARC_FILES += $(OVERWORLD_WILD_BEHAVIOR_PROJECTION_BIN)",
    ...
    require(len(entries) == 22, f"a028 source has {len(entries)} members instead of 22")
    ...
    require(entries[21] == "9_21", f"a028 member 21 is {entries[21]} instead of 9_21")
    a028_path = repo / "base/root/a/0/2/8"
    form_path = repo / "build/a028/9_11"
    parent_path = repo / "build/a028/9_20"
    projection_path = repo / "build/a028/9_21"
    ...
    require(projection_path.is_file(), f"overworld behavior projection is missing: {projection_path}")
    require(
    ...
    require(
        len(a028_members) == len(entries) == 22,
    ...
    reports.append(
        ...
        f"a028[20]=9_20=0x{len(a028_members[20]):X}; "
        f"a028[21]=9_21=0x{len(a028_members[21]):X}/22 members"
    )
