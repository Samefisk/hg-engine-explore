...
def mount_state(emu):
    return {
        ...
        "start_x": signed(emu, MOUNT + 0x96, 2),
        "start_y": signed(emu, MOUNT + 0x98, 2),
        "target_x": signed(emu, MOUNT + 0x9A, 2),
        "target_y": signed(emu, MOUNT + 0x9C, 2),
    }


def wait_until(emu, predicate, limit, mask=0):
    ...
    for motion in motions:
        ...
        summaries.append({
            ...
            "pair_synced": all(
                ...
                and event["rider_face_y"] - event["mount_face_y"] == 0x8000
...
def scenario_stomp():
    ...
    result["passed"] = (
        ...
        and feedback["positive"]["particles"].count(follower) == 1
        and feedback["negative"]["sounds"].count(2183) == 0
        and feedback["negative"]["particles"].count(follower) == 0
    )
    return result
...
def scenario_crash():
    ...
    result["passed"] = (
        ...
        and result["elapsed"] == list(range(1, 33))
        and any(
...
def run_all(include_mankey_save):
    ...
    scenario_names = [
        ...
        or name not in ("mankey_hops", "mounted_transition")
    ]
