...
#define OW_WILD_FOLLOWER_RELEASE_FRAMES 6
#define OW_WILD_FOLLOWER_RETURN_FRAMES 12
#define OW_WILD_FOLLOWER_WHITE_WAIT_FRAMES 16
#define OW_WILD_FOLLOWER_RELEASE_DISPATCH_WHITE 0
#define OW_WILD_FOLLOWER_RELEASE_DISPATCH_RESTORE 1
#define OW_WILD_FOLLOWER_RELEASE_DISPATCH_CANCEL 2
#define OW_WILD_FOLLOWER_RELEASE_DISPATCH_REAPPLY_WHITE 3
    ...
    OverworldWildFollowerReleaseRotationCallback rotate)
{
    ...
    if (frame == 0) {
        ...
        dispatch(fieldSystem, OW_WILD_FOLLOWER_RELEASE_DISPATCH_WHITE);
        if (projectile->opaquePointers[5] == 0) {
            if (++projectile->shakeChecks
                < OW_WILD_FOLLOWER_WHITE_WAIT_FRAMES) {
                return TRUE;
            }
            /* Presentation allocation failed; finish without the flash. */
    ...
    if (frame < OW_WILD_FOLLOWER_RELEASE_FRAMES) {
        if (frame != 0) {
            dispatch(
                fieldSystem,
                OW_WILD_FOLLOWER_RELEASE_DISPATCH_REAPPLY_WHITE);
        }
        follower->faceVec[1] = (u32)(projectile->startHeight
            * (OW_WILD_FOLLOWER_RELEASE_FRAMES - frame)
            / OW_WILD_FOLLOWER_RELEASE_FRAMES);
    ...
    if (frame < OW_WILD_FOLLOWER_RETURN_FRAMES) {
        returnProgress = sOverworldWildFollowerReturnProgress[frame];
        ball->posVec[0] = (u32)(projectile->targetX
            + (((projectile->startX - projectile->targetX)
                * (s32)returnProgress) >> 8));
