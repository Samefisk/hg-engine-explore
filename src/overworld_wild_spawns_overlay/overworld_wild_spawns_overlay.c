...
#define OW_WILD_CUSTOM_JUMP_SHADOW_OBJECT_ID_START 0xA0
...
#define OW_WILD_CUSTOM_JUMP_SHADOW_SPRITE_ID 230 // nearly-empty native shadow carrier tag
#define OW_WILD_CUSTOM_JUMP_SHADOW_BODY_HIDE_Y_FX32 0x000F0000
...
#define OW_WILD_SPAWNER_CUSTOM_JUMP_LANDING_ANIM_BITS 0x00020028
#define OW_WILD_SPAWNER_CUSTOM_JUMP_PRESENTATION_BITS \
    (BIT_JUMP_START | BIT_MOVE_START | MAPOBJECTFLAG_UNK4 | MAPOBJECTFLAG_UNK8 \
        | MAPOBJECTFLAG_UNK13 | OW_WILD_SPAWNER_CUSTOM_JUMP_LANDING_ANIM_BITS)
#define OW_WILD_SPAWNER_CUSTOM_JUMP_LANDING_ANIM_FRAMES 2
...
#define OW_WILD_SPAWNER_THROW_CARRIED_Y_OFFSET_FX32 (OW_WILD_SPAWNER_TILE_FX32 / 2)
...
#define OW_WILD_BEHAVIOR_CLASS_SWARM 5
#define OW_WILD_BEHAVIOR_CLASS_CANOPY_HOPPER 6
#define OW_WILD_BEHAVIOR_CLASS_PICKED_UP 7
...
typedef struct OverworldWildOverlayRuntimeState {
    ...
    u8 movementCustomJumpLandingAnimFrames[OW_WILD_MAX_SPAWNS];
    ...
    LocalMapObject *movementCustomJumpShadowObjects[OW_WILD_MAX_SPAWNS];
    ...
    u8 movementThrowTargets[OW_WILD_MAX_SPAWNS];
    u16 movementThrowTargetMask;
...
static u8 sOverworldWildDataLoadAttemptedFlags;

#define OW_WILD_DATA_LOAD_ATTEMPTED_BEHAVIOR 0x01
#define OW_WILD_DATA_LOAD_ATTEMPTED_ENCOUNTER_LOOKUP 0x02
...
static void OverworldWildSpawns_CleanupResidentData(void)
{
    ...
    sOverworldWildDataLoadAttemptedFlags = 0;
...
{
    if ((sOverworldWildDataLoadAttemptedFlags & OW_WILD_DATA_LOAD_ATTEMPTED_BEHAVIOR) == 0) {
        sOverworldWildDataLoadAttemptedFlags |= OW_WILD_DATA_LOAD_ATTEMPTED_BEHAVIOR;
...
{
    if ((sOverworldWildDataLoadAttemptedFlags & OW_WILD_DATA_LOAD_ATTEMPTED_ENCOUNTER_LOOKUP) == 0) {
        sOverworldWildDataLoadAttemptedFlags |= OW_WILD_DATA_LOAD_ATTEMPTED_ENCOUNTER_LOOKUP;
        if (!OverworldWildSpawns_LoadCodeAddonBlob(
...
static BOOL OverworldWildSpawns_TryStartChainPauseAction(
    ...
    int slot,
    const OverworldWildBehaviorProfile *profile);
static void OverworldWildSpawns_StartCarriedThrowTarget(
    OverworldWildSpawnState *state,
    int carrierSlot,
    int targetSlot,
    LocalMapObject *object);
...
static BOOL OverworldWildSpawns_TickCustomJumpRenderSettle(
    ...
    FieldSystem *fieldSystem,
    int slot);
static void OverworldWildSpawns_ClearCustomJumpShadowObject(
    OverworldWildSpawnState *state,
    int slot);
static void OverworldWildSpawns_SyncCustomJumpShadowObject(
    OverworldWildSpawnState *state,
    int slot,
...
static BOOL OverworldWildSpawns_RequestBattleScript(FieldSystem *fieldSystem, LocalMapObject *object);
...
static LocalMapObject *sOverworldWildDeferredBattleObject;
...
static void OverworldWildSpawns_TryPlayAmbientCry(OverworldWildSpawnState *state)
{
    ...
    u8 cooldownTick;
    int chosen;
    u16 throwTargetMask = OW_WILD_RUNTIME(state)->movementThrowTargetMask;
...
static BOOL OverworldWildSpawns_HasFrameMovementWorkForSlot(OverworldWildSpawnState *state, int slot)
{
    ...
    if (OverworldWildSpawns_IsMovementSlotInProgress(state, slot)
        ...
        || OW_WILD_RUNTIME(state)->movementCustomJumpLandingAnimFrames[slot] != 0
    ...
    if (state->movementSpotStates[slot] == OW_WILD_SPAWNER_SPOT_STATE_CHILL) {
        return (OW_WILD_RUNTIME(state)->movementFrameDrivenChillMask & slotMask) != 0;
    }
...
static void OverworldWildSpawns_RestorePickedUpBehaviorClass(OverworldWildSpawnState *state, int slot)
{
    if (state->movementBehaviorClasses[slot] == OW_WILD_BEHAVIOR_CLASS_PICKED_UP
        ...
        state->movementBehaviorClasses[slot] = state->movementRamSpeeds[slot] - 1;
        state->movementRamSpeeds[slot] = 0;
    }
}
...
static void OverworldWildSpawns_ClearThrowStateForSlot(OverworldWildSpawnState *state, int slot)
{
    ...
    runtime->movementThrowTargetMask &= ~OW_WILD_SPAWNER_MOVEMENT_SLOT_MASK(slot);
    OverworldWildSpawns_RestorePickedUpBehaviorClass(state, slot);
    for (i = 0; i < OW_WILD_MAX_SPAWNS; i++) {
        u8 throwTarget = runtime->movementThrowTargets[i];
        u8 target;

        if (throwTarget == OW_WILD_SPAWNER_THROW_TARGET_NONE) {
            continue;
        }
        target = OW_WILD_SPAWNER_THROW_TARGET_DECODE(throwTarget);
        if (target >= OW_WILD_MAX_SPAWNS) {
            runtime->movementThrowTargets[i] = OW_WILD_SPAWNER_THROW_TARGET_NONE;
            continue;
        }
        if (i == slot
            || target == slot) {
            u16 slotMask = OW_WILD_SPAWNER_MOVEMENT_SLOT_MASK(target);

            runtime->movementThrowTargetMask &= ~slotMask;
            if (throwTarget & OW_WILD_SPAWNER_THROW_TARGET_CARRIED_FLAG) {
                LocalMapObject *object = state->spawns[target].object;

                OverworldWildSpawns_RestorePickedUpBehaviorClass(state, target);
                if (object != NULL) {
...
static BOOL OverworldWildSpawns_IsStablePickupThrowTarget(
    OverworldWildSpawnState *state,
    int carrierSlot,
    int targetSlot)
{
    ...
    if (state == NULL
        ...
        || OverworldWildSpawns_IsMovementSlotInProgress(state, targetSlot)) {
        return FALSE;
    }

    fieldSystem = state->movementFieldSystem;
    object = state->spawns[targetSlot].object;
    return OverworldWildSpawns_IsMovementFieldContextCurrent(state, fieldSystem)
        && OverworldWildSpawns_IsCurrentSpawnObject(fieldSystem, &state->spawns[targetSlot])
        && !MapObject_IsSingleMovementActive(object)
    ...
    int targetSlot)
{
    ...
    if (state == NULL
        ...
        || targetSlot < 0
        || targetSlot >= OW_WILD_MAX_SPAWNS
        || !state->spawns[carrierSlot].active
        || !state->spawns[targetSlot].active
        || state->spawns[carrierSlot].object == NULL
        || state->spawns[targetSlot].object == NULL) {
        return;
    }
    ...
    targetObject->faceVec[1] = carrierObject->faceVec[1] + OW_WILD_SPAWNER_THROW_CARRIED_Y_OFFSET_FX32;
    targetObject->faceVec[2] = carrierObject->faceVec[2];
    targetObject->unk88[0] = carrierObject->unk88[0];
    targetObject->unk88[1] = carrierObject->unk88[1] + OW_WILD_SPAWNER_THROW_CARRIED_Y_OFFSET_FX32;
    ...
    int slot)
{
    ...
    runtime = OW_WILD_RUNTIME(state);
    if (runtime->movementThrowTargets[slot] != OW_WILD_SPAWNER_THROW_TARGET_NONE) {
        return;
    }

    for (i = 0; i < OW_WILD_MAX_SPAWNS; i++) {
        if (OverworldWildSpawns_IsStablePickupThrowTarget(state, slot, i)
            && runtime->movementThrowTargets[i] == OW_WILD_SPAWNER_THROW_TARGET_NONE) {
            u16 slotMask = OW_WILD_SPAWNER_MOVEMENT_SLOT_MASK(i);

            if (runtime->movementThrowTargetMask & slotMask) {
                continue;
            }
            runtime->movementThrowTargets[slot] = OW_WILD_SPAWNER_THROW_TARGET_ENCODE(i);
            runtime->movementThrowTargetMask |= slotMask;
            return;
        }
    }
    ...
    LocalMapObject *object)
{
    ...
    OverworldWildSpawns_ClearMovementSlotInProgress(state, targetSlot);
    OverworldWildSpawns_ClearSpawnRunState(state, targetSlot);
    OverworldWildSpawns_ClearStagedHopTarget(state, targetSlot);
    OverworldWildSpawns_ClearCustomJumpShadowObject(state, targetSlot);
    OverworldWildSpawns_ClearRamCrashMovementState(state, targetSlot);
    state->movementRamSpeeds[targetSlot] = state->movementBehaviorClasses[targetSlot] + 1;
    state->movementBehaviorClasses[targetSlot] = OW_WILD_BEHAVIOR_CLASS_PICKED_UP;
    MapObject_SetBits(object, MAPOBJECTFLAG_UNK18);
    MapObject_ClearBits(object, BIT_VANISH);
    OW_WILD_RUNTIME(state)->movementThrowTargets[carrierSlot] =
        OW_WILD_SPAWNER_THROW_TARGET_ENCODE_CARRIED(targetSlot);
    OverworldWildSpawns_SyncCarriedThrowTarget(state, carrierSlot, targetSlot);
}
...
static void OverworldWildSpawns_ClearCustomJumpPresentationState(OverworldWildSpawnState *state, int slot)
{
    ...
    object = state->spawns[slot].object;
    if (object != NULL) {
        MapObject_ClearBits(object, OW_WILD_SPAWNER_CUSTOM_JUMP_PRESENTATION_BITS);
    }
}
...
static void OverworldWildSpawns_SetObjectLandingTile(LocalMapObject *object, int x, int y)
{
    ...
    MapObject_ClearBits(
        object,
        BIT_VANISH | OW_WILD_SPAWNER_CUSTOM_JUMP_PRESENTATION_BITS);
}

static void OverworldWildSpawns_ReleaseThrowTargetAtCurrentTile(
    OverworldWildSpawnState *state,
    int carrierSlot,
    int targetSlot,
    LocalMapObject *targetObject)
...
static void OverworldWildSpawns_PlayCustomJumpLandingFeedback(
    OverworldWildSpawnState *state,
    int slot,
    LocalMapObject *object)
{
    ...
    MapObject_ClearBits(object, OW_WILD_SPAWNER_CUSTOM_JUMP_PRESENTATION_BITS);
    MapObject_SetBits(object, OW_WILD_SPAWNER_CUSTOM_JUMP_LANDING_ANIM_BITS);
    OW_WILD_RUNTIME(state)->movementCustomJumpLandingAnimFrames[slot] =
        OW_WILD_SPAWNER_CUSTOM_JUMP_LANDING_ANIM_FRAMES;
}

static void OverworldWildSpawns_TickCustomJumpLandingFeedback(
    OverworldWildSpawnState *state,
    FieldSystem *fieldSystem,
    int slot)
{
    ...
    if (runtime->movementCustomJumpActive[slot]
        || runtime->movementCustomJumpLandingAnimFrames[slot] == 0) {
        return;
    }
    ...
    if (--runtime->movementCustomJumpLandingAnimFrames[slot] == 0) {
        MapObject_ClearBits(object, OW_WILD_SPAWNER_CUSTOM_JUMP_LANDING_ANIM_BITS);
    }
}
...
static BOOL OverworldWildSpawns_IsCustomJumpShadowObjectId(int id)
{
    return id >= OW_WILD_CUSTOM_JUMP_SHADOW_OBJECT_ID_START
        && id < OW_WILD_CUSTOM_JUMP_SHADOW_OBJECT_ID_START + OW_WILD_MAX_SPAWNS;
}
...
static BOOL OverworldWildSpawns_IsIgnoredVisualObjectId(int id)
{
    return OverworldWildSpawns_IsPhantomFlickerObjectId(id)
        ...
        || OverworldWildSpawns_IsCustomJumpShadowObjectId(id);
}
    ...
    int slot)
{
    ...
    shadowObject = runtime->movementCustomJumpShadowObjects[slot];
    runtime->movementCustomJumpShadowObjects[slot] = NULL;
    ...
    fieldSystem = state->movementFieldSystem;
    if (OverworldWildSpawns_IsMovementFieldContextCurrent(state, fieldSystem)
        && OverworldWildSpawns_IsCurrentMapObject(fieldSystem, shadowObject)) {
        DeleteMapObject(shadowObject);
        OW_WILD_PERF_INC(sOverworldWildPerfMapObjectDeletesThisFrame);
    }
}
    ...
    LocalMapObject *sourceObject)
{
    ...
    currentX = (int)(sourceObject->posVec[0] >> 16);
    currentY = (int)(sourceObject->posVec[2] >> 16);
    currentBehavior = GetMetatileBehaviorAt(fieldSystem, currentX, currentY);
    if (currentBehavior == OW_WILD_TILE_ENCOUNTER_GRASS
        || currentBehavior == OW_WILD_TILE_LONG_GRASS
        || currentBehavior == OW_WILD_TILE_HEADBUTT
        || OverworldWildSpawns_IsSurfBehavior(currentBehavior)) {
        OverworldWildSpawns_ClearCustomJumpShadowObject(state, slot);
        return;
    }

    shadowObject = runtime->movementCustomJumpShadowObjects[slot];
    if (shadowObject != NULL) {
        if (OverworldWildSpawns_IsCurrentMapObject(fieldSystem, shadowObject)) {
            goto sync;
        }
        runtime->movementCustomJumpShadowObjects[slot] = NULL;
    }

    shadowObject = CreateSpecialFieldObjectWithParams(
        fieldSystem->mapObjectMan,
        sourceObject->xCurr,
        sourceObject->yCurr,
        sourceObject->curFacing,
        OW_WILD_CUSTOM_JUMP_SHADOW_SPRITE_ID,
    ...
    runtime->movementCustomJumpShadowObjects[slot] = shadowObject;
    shadowObject->id = OW_WILD_CUSTOM_JUMP_SHADOW_OBJECT_ID_START + slot;
    shadowObject->flags |= MAPOBJECTFLAG_UNK18;

sync:
    shadowObject->xCurr = sourceObject->xCurr;
    shadowObject->yCurr = sourceObject->yCurr;
    shadowObject->hCurr = sourceObject->hCurr;
    shadowObject->posVec[0] = sourceObject->posVec[0];
    shadowObject->posVec[1] = sourceObject->posVec[1];
    shadowObject->posVec[2] = sourceObject->posVec[2];
    shadowObject->faceVec[1] = OW_WILD_CUSTOM_JUMP_SHADOW_BODY_HIDE_Y_FX32;
    shadowObject->unk88[1] = OW_WILD_CUSTOM_JUMP_SHADOW_BODY_HIDE_Y_FX32;
    shadowObject->flags &= ~BIT_VANISH;
}
    ...
    LocalMapObject *object)
{
    ...
    MapObject_SetScript(replacement, OVERWORLD_WILD_SPAWNS_BATTLE_SCRIPT);
    profile = OverworldWildSpawns_GetBehaviorProfile(state, slot);
    range = profile.range;
    if (range == 0) {
        range = OW_WILD_SPAWNER_MOVEMENT_RANGE;
...
static void OverworldWildSpawns_ResetSlotMovementCommand(OverworldWildSpawnState *state, int slot, BOOL clearObjectCommand)
{
    ...
    if (clearObjectCommand
        ...
        MapObject_ClearBits(
            state->spawns[slot].object,
            OW_WILD_SPAWNER_CUSTOM_JUMP_PRESENTATION_BITS);
        if (MapObject_IsSingleMovementActive(state->spawns[slot].object)) {
            MapObject_ClearSingleMovementActive(state->spawns[slot].object);
        }
    }
...
static void OverworldWildSpawns_DetachAllMovementStateOnContextLoss(OverworldWildSpawnState *state)
{
    ...
    for (i = 0; i < OW_WILD_MAX_SPAWNS; i++) {
        ...
        OverworldWildSpawns_ClearStagedHopMovementListTask(state, i);
        OverworldWildSpawns_ClearCustomJump(state, i);
        OverworldWildSpawns_ClearCustomJumpShadowObject(state, i);
        ...
        runtime->movementBehaviorLimitKeys[i] = OW_WILD_BEHAVIOR_CLASS_DEFAULT;
        ...
        state->movementStagedHopPending[i] = FALSE;
        state->movementStagedHopDistances[i] = 0;
    ...
    const OverworldWildBehaviorPrimitives *primitives)
{
    ...
    runtime = OW_WILD_RUNTIME(state);
    throwTarget = runtime->movementThrowTargets[slot];
    if (throwTarget == OW_WILD_SPAWNER_THROW_TARGET_NONE
        && primitives->attentiveLocomotion == OW_WILD_BEHAVIOR_LOCOMOTION_HOP
        && primitives->attentiveTarget == OW_WILD_BEHAVIOR_TARGET_TREE_TOP) {
        OverworldWildSpawns_TryStartHeadbuttTreeHop(state, fieldSystem, slot);
        return TRUE;
    }
    ...
    if (throwTarget & OW_WILD_SPAWNER_THROW_TARGET_CARRIED_FLAG) {
        ...
        if (throwTarget & OW_WILD_SPAWNER_THROW_TARGET_WINDUP_FLAG) {
            ...
            if (throwDistance != 0
                && OverworldWildSpawns_StartPreparedCustomJumpCommand(
                    ...
                    profile,
                    TRUE)) {
                runtime->movementThrowTargets[slot] = OW_WILD_SPAWNER_THROW_TARGET_NONE;
        ...
        if ((dx == 0 || dy == 0)
            ...
            runtime->movementThrowTargets[slot] =
    ...
    if (throwTarget != OW_WILD_SPAWNER_THROW_TARGET_NONE
        ...
        if (directionCount == 0
            ...
            OverworldWildSpawns_StartCarriedThrowTarget(state, slot, targetSlot, targetObject);
            return TRUE;
        }
    }
    if (directionCount == 0) {
    ...
    if (primitives->attentiveLocomotion == OW_WILD_BEHAVIOR_LOCOMOTION_HOP) {
        if (throwTarget != OW_WILD_SPAWNER_THROW_TARGET_NONE
            && (throwTarget & OW_WILD_SPAWNER_THROW_TARGET_CARRIED_FLAG) == 0) {
            targetObject->flags &= ~MAPOBJECTFLAG_ACTIVE;
            movementTarget = OW_WILD_BEHAVIOR_TARGET_NONE;
        }
...
static void OverworldWildSpawns_PlayRamCrashThud(OverworldWildSpawnState *state)
{
    ...
    GF_Snd_LoadSeqEx(OW_WILD_SPAWNER_ONIX_RAM_CRASH_SE, NNS_SND_ARC_LOAD_ALL);
    ...
    int targetY)
{
    ...
    if (sOverworldWildMovementFrameTaskExecuting) {
        ...
        if (!OverworldWildSpawns_RequestBattleScript(fieldSystem, object)) {
            OverworldWildSpawns_ResetPendingBattle(state);
            return FALSE;
        }
        return TRUE;
...
static void OverworldWildSpawns_PlayRamStepFeedback(OverworldWildSpawnState *state, int slot, LocalMapObject *object)
{
    ...
    GF_Snd_LoadSeqEx(OW_WILD_SPAWNER_ONIX_RAM_STEP_SE, NNS_SND_ARC_LOAD_ALL);
...
static void OverworldWildSpawns_StartTiredEmote(OverworldWildSpawnState *state, int slot)
{
    OverworldWildSpawns_StartTiredEmoteWithProfile(state, slot, NULL);
}
    ...
    const OverworldWildBehaviorPrimitives *primitives)
{
    if (state == NULL
        ...
        || slot < 0
        || slot >= OW_WILD_MAX_SPAWNS
        || profile->ramAccelerationSteps == 0
    ...
    if (++state->movementRamStepCounters[slot] >= profile->ramAccelerationSteps) {
        state->movementRamStepCounters[slot] = 0;
        state->movementCooldowns[slot] = profile->ramMaxSpeed;
...
static void OverworldWildSpawns_HandleFinishedMovementCommand(OverworldWildSpawnState *state, int slot)
{
    ...
    if (object != NULL) {
        ...
        if ((runtime->movementThrowTargetMask & slotMask) != 0) {
            runtime->movementThrowTargetMask &= ~slotMask;
            OverworldWildSpawns_RestorePickedUpBehaviorClass(state, slot);
            (void)OverworldWildSpawns_RunImmediateCanopyMovementCommand(
                object,
            ...
            if (runtime->movementCustomJumpActive[slot]
                ...
                OverworldWildSpawns_ClearCustomJump(state, slot);
                OverworldWildSpawns_PlayCustomJumpLandingFeedback(state, slot, object);
            }
    ...
    BOOL frameTick)
{
    ...
    if (!frameTick && runtime->movementThrowTargetMask == 0) {
        ...
        if (state->spawns[i].active && state->spawns[i].object != NULL) {
            ...
            if (OverworldWildSpawns_IsMovementSlotInProgress(state, i)) {
                ...
                if (OverworldWildSpawns_UpdateSpawnerMovementCommandForSlot(state, i)) {
                    ...
                    if (!OverworldWildSpawns_HandleFinishedStagedHopMovementCommand(state, fieldSystem, i)) {
            ...
            if (runtime->movementThrowTargetMask & OW_WILD_SPAWNER_MOVEMENT_SLOT_MASK(i)) {
                continue;
            }
            ...
            if (state->movementSpotStates[i] == OW_WILD_SPAWNER_SPOT_STATE_ACTIVE) {
                if (shouldIssueLookCommand) {
                    ...
                    if (OverworldWildSpawns_TryStartFrameDrivenActiveMovementCommand(
                            state,
                            i,
                            object,
                            &profile,
            ...
            if (fieldSystem != NULL && fieldSystem->playerAvatar != NULL) {
                ...
                {
                    BOOL canSpotPlayer =
                        state->movementSpotStates[i] == OW_WILD_SPAWNER_SPOT_STATE_CHILL
                    ...
                    if (shouldIssueLookCommand
                        ...
                        && movementTarget == OW_WILD_BEHAVIOR_TARGET_TREE_TOP) {
                        OverworldWildSpawns_TryStartHeadbuttTreeHop(state, fieldSystem, i);
                    }
...
static void OverworldWildSpawns_SyncCustomJumpLogicalTile(LocalMapObject *object)
{
    ...
    LocalMapObject *object)
{
    ...
    OverworldWildSpawns_SetObjectRenderTileOnly(
        ...
        OW_WILD_RUNTIME(state)->movementCustomJumpTargetX[slot],
        OW_WILD_RUNTIME(state)->movementCustomJumpTargetY[slot]);
    MapObject_ClearBits(object, BIT_VANISH | OW_WILD_SPAWNER_CUSTOM_JUMP_PRESENTATION_BITS);
    sOverworldWildCustomJumpRenderSettleActive[slot] = FALSE;
    ...
    u32 elapsed)
{
    ...
    object->unk94[1] = 0;
#if OW_WILD_SPAWNER_CUSTOM_JUMP_SYNC_LOGICAL_SECONDARY
    OverworldWildSpawns_SyncCustomJumpLogicalTile(object);
#endif
    OverworldWildSpawns_SyncCustomJumpShadowObject(state, slot, object);
    ...
    BOOL commandFinished)
{
    ...
    if (elapsed < total) {
        ...
        OverworldWildSpawns_ApplyCustomJumpRenderOffset(state, slot, object, elapsed);
    }
    if (elapsed < total) {
        return FALSE;
    }
...
#if OW_WILD_SPAWNER_CUSTOM_JUMP_RENDER_OFFSET
        OverworldWildSpawns_ApplyCustomJumpRenderOffset(state, slot, object, total);
#endif
...
static BOOL OverworldWildSpawns_UpdateSpawnerMovementCommandForSlot(OverworldWildSpawnState *state, int slot)
{
    ...
    return OverworldWildSpawns_UpdateCustomJumpLanding(state, slot, object, commandFinished);
}
#endif
...
static void OverworldWildSpawns_CancelDeferredBattleScript(void)
{
    if (sOverworldWildDeferredBattleScriptTask != NULL) {
        ...
        sOverworldWildDeferredBattleObject = NULL;
        DestroySysTask(task);
        return;
    }
...
static void OverworldWildSpawns_DeferredBattleScriptTask(SysTask *task, void *data)
{
    ...
    object = sOverworldWildDeferredBattleObject;
    sOverworldWildDeferredBattleScriptTask = NULL;
    sOverworldWildDeferredBattleFieldSystem = NULL;
    sOverworldWildDeferredBattleObject = NULL;
...
static BOOL OverworldWildSpawns_DeferBattleScript(FieldSystem *fieldSystem, LocalMapObject *object)
{
    if (fieldSystem != NULL
    ...
    sOverworldWildDeferredBattleObject = object;
    if (sOverworldWildDeferredBattleScriptTask == NULL) {
...
static void OverworldWildSpawns_FrameMovementTask(SysTask *task, void *data)
{
    ...
    for (i = 0; i < OW_WILD_MAX_SPAWNS; i++) {
        ...
        if (currentSpawnObject) {
            ...
            OverworldWildSpawns_TickCustomJumpLandingFeedback(state, fieldSystem, i);
...
#if OW_WILD_SPAWNER_MOVEMENT_DIAGNOSTIC_FRAME_TASK
    return OverworldWildSpawns_DeferBattleScript(fieldSystem, object);
#else
    return OverworldWildSpawns_QueueBattleScriptTask(fieldSystem, object);
#endif
}
...
static BOOL OverworldWildSpawns_IsStaticFishingShoreCandidate(FieldSystem *fieldSystem, int x, int y)
{
    u8 behavior;
...
{
    return OverworldWildSpawns_IsStaticFishingShoreCandidate(fieldSystem, x, y)
...
#if OW_WILD_SPAWNER_MOVEMENT_DIAGNOSTIC_PARAM_TICK
static BOOL OverworldWildSpawns_IsStagedHopAvoidTarget(
    OverworldWildSpawnState *state,
    int slot,
    int targetX,
    int targetY)
{
    ...
    return state->movementStagedHopAvoidX[slot] == targetX
        && state->movementStagedHopAvoidY[slot] == targetY;
}
    ...
    u32 *candidateCount)
{
    ...
    if (!OverworldWildSpawns_TryGetBehaviorHopVector(
        ...
        || (!ignoreAvoidTarget && OverworldWildSpawns_IsStagedHopAvoidTarget(state, slot, treeX, treeY))
    ...
    int targetY)
{
    ...
    distance = OverworldWildSpawns_Max(
        OverworldWildSpawns_Abs(dx),
        OverworldWildSpawns_Abs(dy));
    if (distance <= 1 || !OverworldWildSpawns_IsCustomJumpVectorShape(dx, dy)) {
        return FALSE;
    }
    ...
    for (avoidPass = 0; avoidPass < 2 && candidateCount == 0; avoidPass++) {
        ...
        while (OverworldWildSpawns_NextHeadbuttTreeTopCandidate(
            ...
            if (!OverworldWildSpawns_TryGetBehaviorHopVector(
                    ...
                    &distance)
                || (!ignoreAvoidTarget
                    && OverworldWildSpawns_IsStagedHopAvoidTarget(state, slot, candidateX, candidateY))) {
                continue;
            }
    ...
    BOOL *stepIsFinalTarget)
{
    ...
    while (head < tail) {
        ...
        for (offsetIndex = 0; offsetIndex < NELEMS(sCustomJumpOffsets); offsetIndex++) {
            int dx = sCustomJumpOffsets[offsetIndex].dx;
            int dy = sCustomJumpOffsets[offsetIndex].dy;
            int distance;
            ...
            for (distance = OverworldWildSpawns_GetBehaviorHopMaxDistance(
                ...
                if (!finalTarget
                    && (!OverworldWildSpawns_IsWalkableLandTile(fieldSystem, nextX, nextY)
                        ...
                        || OverworldWildSpawns_IsStagedHopAvoidTarget(state, slot, nextX, nextY))) {
                    continue;
                }
...
#endif
#if OW_WILD_SPAWNER_CANOPY_SHIFT_NORTH_LAND_ANCHORS
    if (OverworldWildSpawns_TryGetShiftedNorthLandTreeTop(
            fieldSystem,
            currentX,
            currentY,
            targetX,
    ...
    BOOL suppressHopStartSound)
{
    ...
    if (!OverworldWildSpawns_RunImmediateCanopyMovementCommand(
        ...
        (void)OverworldWildSpawns_RunImmediateCanopyMovementCommand(
            object,
            OW_WILD_SPAWNER_CANOPY_HOPPER_PARTNER_RESTORE_COMMAND);
        if (!suppressHopStartSound || (object->flags & MAPOBJECTFLAG_UNK18) == 0) {
            return FALSE;
        }
    }
    ...
    BOOL *finished)
{
    ...
    OverworldWildSpawns_ClearStagedHopMovementListTask(state, slot);
    OverworldWildSpawns_ClearMovementSlotInProgress(state, slot);
    if (!OverworldWildSpawns_HandleFinishedStagedHopMovementCommand(state, fieldSystem, slot)) {
        OverworldWildSpawns_HandleFinishedMovementCommand(state, slot);
    }
    if (finished != NULL) {
        *finished = TRUE;
    ...
    int slot)
{
    ...
    customJumpRenderSettleActive =
        OverworldWildSpawns_FinalizeCustomJumpAfterRestore(state, slot, object);
#endif
    ...
    || OW_WILD_SPAWNER_CANOPY_TREE_ANCHOR_PARTNER_PREPPED_INTERNAL_JUMP_PROBE
{
    ...
    if (state->movementStagedHopPending[slot]) {
        ...
        return OverworldWildSpawns_ExecutePendingStagedHop(state, fieldSystem, slot, object, &profile);
    }
    if (MapObject_IsSingleMovementActive(object)) {
        return FALSE;
    }
    ...
    OverworldWildSpawnStartup *startup)
{
    ...
    startY = targetY - OverworldWildSpawns_MovementDirectionDeltaY(direction)
        * OW_WILD_SPAWNER_SPAWN_HOP_DISTANCE;
    if (!OverworldWildSpawns_IsSpawnHopStartTile(fieldSystem, profile, startX, startY)) {
        return FALSE;
    }
    ...
    OverworldWildSpawnStartup *startup)
{
    ...
    if (primitives.spawnLocomotion == OW_WILD_BEHAVIOR_LOCOMOTION_RUN_FROM_OFF_SCREEN) {
        ...
        (void)OverworldWildSpawns_PrepareSpawnHopStart(fieldSystem, profile, startup);
...
static BOOL OverworldWildSpawns_HelperIsWalkableLandTile(void *context, int x, int y)
{
    ...
    return OverworldWildSpawns_IsWalkableLandTile(prepContext->fieldSystem, x, y);
}

static BOOL OverworldWildSpawns_HelperIsStaticFishingShoreTile(void *context, int x, int y)
{
    OverworldWildSpawnPrepContext *prepContext = (OverworldWildSpawnPrepContext *)context;

    return OverworldWildSpawns_IsStaticFishingShoreCandidate(prepContext->fieldSystem, x, y);
}
...
static const OverworldWildHelperSpawnCallbacks sOverworldWildSpawnPrepCallbacks = {
    ...
    OverworldWildSpawns_HelperIsStaticFishingShoreTile,
...
static void OverworldWildSpawns_DespawnFarMons(OverworldWildSpawnState *state, FieldSystem *fieldSystem)
{
    ...
    for (i = 0; i < OW_WILD_MAX_SPAWNS; i++) {
        if (state->spawns[i].active && state->spawns[i].object != NULL) {
            ...
            if (((OW_WILD_RUNTIME(state)->movementThrowTargetMask | state->movementInProgressMask)
                    & OW_WILD_SPAWNER_MOVEMENT_SLOT_MASK(i)) == 0
                && !state->movementSpawnRunActive[i]
                && !state->spawns[i].shiny
                && OverworldWildSpawns_DistanceFromPlayer(fieldSystem, x, y) > OW_WILD_DESPAWN_CULL_DISTANCE) {
...
#if OW_WILD_SPAWNER_MOVEMENT_DIAGNOSTIC_PARAM_TICK
    if (OW_WILD_RUNTIME(state)->movementThrowTargetMask & OW_WILD_SPAWNER_MOVEMENT_SLOT_MASK(slot)) {
        return FALSE;
    }
    if (state->movementSpawnRunActive[slot]) {
        return FALSE;
...
static BOOL OverworldWildSpawns_IsSlotStableForBattle(OverworldWildSpawnState *state, int slot)
{
    ...
    object = state->spawns[slot].object;
    if ((OW_WILD_RUNTIME(state)->movementThrowTargetMask & OW_WILD_SPAWNER_MOVEMENT_SLOT_MASK(slot)) != 0
        || state->movementSpawnRunActive[slot]
        || state->movementStagedHopPending[slot]
        || state->movementRamCrashShakeTimers[slot] != 0
        || state->movementSpotStates[slot] == OW_WILD_SPAWNER_SPOT_STATE_EMOTING
    ...
    int slot)
{
    ...
    if (!OverworldWildSpawns_RequestBattleScript(fieldSystem, state->spawns[slot].object)) {
        OverworldWildSpawns_ResetPendingBattle(state);
        return FALSE;
    }
    return TRUE;
    ...
    int slot)
{
    if (state == NULL
        ...
        || slot >= OW_WILD_MAX_SPAWNS
        || OverworldWildSpawns_HasPendingBattle(state)
        || (OW_WILD_RUNTIME(state)->movementThrowTargetMask & OW_WILD_SPAWNER_MOVEMENT_SLOT_MASK(slot)) != 0
        || !OverworldWildSpawns_IsMovementFieldContextCurrent(state, fieldSystem)
        || !OverworldWildSpawns_IsCurrentSpawnObject(fieldSystem, &state->spawns[slot])) {
        return FALSE;
    }
...
static int OverworldWildSpawns_FindTalkedObjectSlot(
    FieldSystem *fieldSystem,
    OverworldWildSpawnState *state,
    LocalMapObject *talkedObject)
{
    ...
    LocalMapObject *talkedObject)
{
    ...
    playerObject = fieldSystem->playerAvatar->mapObject;
    i = OverworldWildSpawns_FindTalkedObjectSlot(fieldSystem, state, talkedObject);
    if (i >= 0) {
        return i;
    }
    ...
    playerX = GetPlayerXCoord(fieldSystem->playerAvatar);
    playerY = GetPlayerYCoord(fieldSystem->playerAvatar);
    targetX = playerX + OverworldWildSpawns_MovementDirectionDeltaX(facing);
    targetY = playerY + OverworldWildSpawns_MovementDirectionDeltaY(facing);

    for (i = 0; i < OW_WILD_MAX_SPAWNS; i++) {
        LocalMapObject *spawnObject = state->spawns[i].object;

        if (state->movementSpawnRunActive[i]
            || (OW_WILD_RUNTIME(state)->movementThrowTargetMask & OW_WILD_SPAWNER_MOVEMENT_SLOT_MASK(i)) != 0) {
            continue;
        }
        ...
        {
            int spawnX = MapObject_GetCurrentX(spawnObject);
            int spawnY = MapObject_GetCurrentY(spawnObject);

            if ((spawnX == targetX && spawnY == targetY)
                || (spawnX == playerX && spawnY == playerY)) {
                return i;
            }
        }

        if (previousTileSlot < 0
            && OverworldWildSpawns_IsPreviousTileLockedAt(state, i, targetX, targetY)) {
            previousTileSlot = i;
        }
    }
