"""Exact Wild Sprint obstacle-Hop proof from an idle adjacent tile."""
...
KIND = "wild-sprint-obstacle-approach-v1"
REQUIREMENT = "current.wild-sprint-obstacle-approach"
CLAIMS = ("controlled-action", "live-actor-identity", "profile-resolution",
          "collision-decision", "logical-commit", "rendered-motion",
          "control-release")
ACTIONS = ("direct-east-at-obstacle", "hop-over-obstacle", "close-direction-control")
...
def contract():
    ...
    return {
        ...
        "collision-decision": [exact("open-takeoff-static-front-safe-landing")],
...
def _need(value, reason):
    if not value:
        raise ValueError("Wild Sprint obstacle Hop: " + reason)
...
def _fault(commands, frames, events, name):
    ...
    if name == "wrong-terrain":
        ...
        _actor(commands[ACTIONS[0]]["snapshot"])["logical"]["x"] = 579
    elif name == "short-hop":
        ...
        raise ValueError("unknown obstacle-Hop copied control")
    return True
...
def _measure(commands, frames, events):
    ...
    _need(initial.get("context", {}).get("mapId") == 33
          ...
          and initial.get("player", {}).get("x") == 584
          and initial.get("player", {}).get("y") == 397,
          "reviewed Route 29 idle takeoff differs")
    cells = {(cell["x"], cell["z"]): cell
             for cell in initial.get("terrain", {}).get("cells", [])}
    _need(all(cells.get((x, 397), {}).get("loaded") is True
              and cells[(x, 397)].get("collision") is blocked
              for x, blocked in ((580, False), (581, True), (582, False))),
          "takeoff, static signpost or landing terrain differs")
    ...
    _need(len(accepted) == 1 and accepted[0].get("stage") == 0
          ...
          "real idle obstacle Hop was not admitted")
    for call in calls:
    ...
    _need(hop is not None
          and hop[1].get("origin") == {"x": 580, "y": 397}
          and hop[1].get("target") == {"x": 582, "y": 397}
          and _actor(frames[-1]).get("logical") == {"x": 582, "y": 397}
    ...
    _need(len(recorder.completed) == 1
          ...
          and recorder.completed[0]["origin"] == [580, 397]
          and recorder.completed[0]["target"] == [582, 397],
    ...
    return {"values": {"east-only-register-control": 1,
                       ...
                       "open-takeoff-static-front-safe-landing": 1,
                       "one-two-tile-hop": 1,
                       "hop-rendered": 1,
                       "hop-finished": 1},
...
def run(rows, test=None, fault=None):
    ...
    try:
        if test is not None:
            _need(test.get("id") == "walk.wild.sprint-obstacle-approach"
                  and [action["id"] for action in test.get("actions", [])] == list(ACTIONS),
                  "reviewed obstacle recipe differs")
