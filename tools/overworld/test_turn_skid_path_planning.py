"""Host invariants for opt-in Wild turn-skid path planning."""
    ...
    u16 allowedTile, int x, int y, int finalTargetX, int finalTargetY)
{
    ...
    return x >= 0 && x < 24 && y >= 0 && y < 24 && !blocked[y][x];
}
...
int main(void)
{
    ...
    Check(!OverworldWildSpawns_StartMomentumWalkStep(&context, &call)
            && startCalls == 0,
        "normal Runner step entered a lane without its future skid runway");
    ...
    Check(OverworldWildSpawns_StartMomentumWalkStep(&context, &turn)
            && startCalls == 1,
        "Runner could not turn after its blocked straight runway was rejected");
    blocked[10][12] = 0;
    Check(OverworldWildSpawns_StartMomentumWalkStep(&context, &call)
            && startCalls == 2,
        "normal Runner step with its future skid runway was rejected");
    ...
    puts("turn-skid path geometry: 9 checks passed");
    return 0;
}
"""
...
class TurnSkidPathPlanningTests(unittest.TestCase):
    def test_full_skid_and_first_post_turn_step_are_required(self) -> None:
    ...
    def test_validation_precedes_facing_and_wander_owns_tile_validation(self) -> None:
        ...
        wander = single.index("== OW_WILD_BEHAVIOR_LOCOMOTION_WANDER")
        self.assertLess(wander, single.index("OverworldWildSpawns_IsBehaviorAllowedMovementTile"))
        self.assertIn("OverworldWildSpawns_TryStartCanopyEntryHop", single[:single.index(
